...
can.test.path = function(path){
	if (typeof steal !== 'undefined') {
		return ""+steal.idToUri(steal.id(path).toString())  ;
	}
...
test("findAll deferred", function(){
	can.TurboModel("Person",{
		findAll : function(params, success, error){
			var self = this;
			return can.ajax({
				url : "/people",
				data : params,
				fixture: can.test.fixture("can-turbomodel/test/people.json"),
				dataType : "json"
			}).pipe(function(data){
...
test("findAll rejects non-array (#384)", function() {
	return expect(0)
	var Person = can.TurboModel.extend({
		findAll : function(params, success, error){
			var dfd = Q.defer();
			setTimeout(function() {
				dfd.resolve({
...
test("findOne deferred", function(){
	if(window.jQuery){
		can.TurboModel("Person",{
			findOne : function(params, success, error){
				...
				return can.ajax({
					url : "/people/5",
					data : params,
					fixture: can.test.fixture("can-turbomodel/test/person.json"),
					dataType : "json"
				}).pipe(function(data){
		...
		can.TurboModel("Person",{
			findOne : can.test.fixture("can-turbomodel/test/person.json")
		},{});
	}
	stop();
...
test(".models with custom id", function() {
	can.TurboModel("CustomId", {
		findAll : can.test.path("can-turbomodel/test/customids.json"),
		id : '_id'
	}, {
		getName : function() {
			return this.name;
...
test("async setters", function(){
	...
	can.TurboModel("Test.AsyncModel",{
...
test("auto methods",function(){
	...
	var School = can.TurboModel.extend("Jquery.Model.Models.School",{
	   findAll : can.test.path("can-turbomodel/test/{type}.json"),
	   findOne : can.test.path("can-turbomodel/test/{id}.json"),
	   create : "GET " + can.test.path("can-turbomodel/test/create.json"),
	   update : "GET "+ can.test.path("can-turbomodel/test/update{id}.json")
	},{})
	stop();
...
test("findAll string", function(){
	can.fixture.on = false;
	can.TurboModel("Test.Thing",{
		findAll : can.test.path("can-turbomodel/test/findAll.json")+''
	},{});
	stop();
...
test("Model events" , function(){
	...
	expect(12)
	var order = 0;
	can.TurboModel("Test.Event",{
		create : function(attrs){
			var def = isDojo ? new dojo.Deferred() : new Q.defer();
			def.resolve({id: 1})
			return def;
		},
...
test("save error args", function(){
	var Foo = can.TurboModel.extend('Testin.Models.Foo',{
...
test(".models updates existing list if passed", 4, function() {
	var Model = can.TurboModel.extend({});
...
test("model removeAttr (#245)", function() {
	var MyModel = can.TurboModel.extend({}),
		model;
	can.TurboModel._reqs++; // pretend it is live bound
	model = MyModel.model({
		id: 0,
		index: 2,
		name: 'test'
...
test(".model on create and update (#301)", function() {
	var MyModel = can.TurboModel.extend({
		...
		update: 'PUT /todo',
		model: function(data) {
			return can.TurboModel.model.call(this, data.item);
		}
	}, {}),
		id = 0,
		updateTime;
...
test("Creating nested models adds them to the store (#357)", function(){
   ...
   // Raw data. Nested object is there twice
	...
	// Company model
	Company = can.TurboModel.extend({},{})

	// Person model with nested Company
	Person = can.TurboModel.extend({
		attributes: {
...
test("destroy not calling callback for new instances (#403)", function(){
	var Recipe = can.TurboModel.extend({},{})
	expect(1);
	stop();
...
test(".model should always serialize Observes (#444)", function() {
	var ConceptualDuck = can.TurboModel.extend({
		defaults: {
...
test("string configurable model and models functions (#128)", function(){
	var StrangeProp = can.TurboModel.extend({
		model: "foo",
		models: "bar"
	},{});
...
test("create deferred does not resolve to the same instance", function(){
	var Todo = can.TurboModel.extend({
	  create: function(){
		var def = new Q.defer();
		def.resolve({id: 5})
		return def;
